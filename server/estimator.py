...
class EstimationCapacity:
  ...
  def __init__(self, path, device_chip, device_date, device_time ):
    print( "Path : " + path)
    self.path = path
    self.estimate()
    self.estimating( device_chip, device_date, device_time )
  ...
  def estimating( self, device_chip, device_date, device_time  ):
